...
音声は書かない。`data/part3/` `data/part4/` `data/part5/` に JSON が入ると
GitHub Actions が読み上げ音声を作り、`audio` を足して別コミットで main に入れる。
...
- **解説で音声が鳴る。** 時事英語と同じく1問ずつのファイルを Actions が作る。
  読み上げるのは空所を正解で埋めた文なので、正しい形のまま耳に入る。
  各問の ▶ で1文、見出しの ▶ で10問を続けて再生できる
