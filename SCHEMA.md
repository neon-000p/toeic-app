...
**Part 3〜7 の語彙ステップの例文**
2026-09-26 以降のセットは、時事の語彙と同じく語注の各語に `"example": { "en": "...", "ja": "..." }` を持つ。本文の文は写さず、同じ語を別の場面に置いた短い例文を作る（`tools/check-part*.js` が確認する）。アプリはこれを語彙ステップに和訳・▶つきで出す。`example` の無い古いセットでは、本文でその語が出てくる最初の文を代わりに出す（`TOEIC.exampleFor()`、空所は正解で埋めた形）。
