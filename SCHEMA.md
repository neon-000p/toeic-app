...
Part 3（会話問題）。1つのパックに会話の「セット」を2〜3本入れる。1セット＝会話1本＋設問3問で、本番と同じ単位。
...
`M` / `W` / `M2` / `W2` の4種。アプリは tag ごとに別の声を割り当てる。`accent`（`en-US` / `en-GB` / `en-AU` / `en-CA`）を手がかりに声を選び、名前から性別が分からない端末では高さで作り分ける。TOEIC が4か国の発音を混ぜるので、データ側でもそこに寄せる。

**`scene` は先読み中に出さない**
何の話かを先に知ると、先読みの練習にならない。復習画面でだけ表示する。

**`evidence` は行番号の配列**
`lines` の添字（0始まり）。復習でその行の左に線が入る。1問が複数行にまたがることがあるので配列。

**`type` は6種**
`gist`（概要）／`detail`（詳細）／`intent`（意図）／`next`（次の行動）／`infer`（推測）／`graphic`（図表）。結果画面で型別の正答率を出し、**低い順に並べる**。どの型が弱いかが分かるようにするための項目なので、必ず入れる。

**`glossary` はセットごと**
その会話に出てくる語だけを持つ。教材全体で共有しない。
...
```json
{
  ...
  "items": [
    { "id": "2026-09-13-0530", "date": "2026-09-13", "time": "05:30", "sets": 2, "file": "2026-09-13-0530.json" }
  ]
}
```
