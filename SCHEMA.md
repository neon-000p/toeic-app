...
```json
"audio": {
  "lines": ["audio/2026-09-18-1258-1/00.mp3", "audio/2026-09-18-1258-1/01.mp3"],
  "voice": "Charon",
  "bytes": 184320
}
```

本番の Part 5 に音声は無いが、正しい形の文を耳に入れられるよう、**時事英語と同じ形**で1文ずつ用意する。読み上げるのは**空所を正解で埋めた文**（ハイフンの並びをそのまま渡すと読み上げが崩れるため）。並びは `questions` と同じで、`lines[i]` が i 問目にあたる。

**ルーティンはこれを書かない。** GitHub Actions（`.github/workflows/part3-audio.yml`）が `tools/make-part5-audio.js` を走らせて作り、`audio` を足して別コミットで入れる。まるごと1本のファイルは作らない（順に鳴らせば同じで、頭出しも要らない）。

アプリは解説ステップで、各問の ▶ で1文だけ、見出しの ▶ で10問を続けて鳴らす。`audio.lines[i]` が無い、あるいは 404 のときは端末の読み上げに落ちるので、音声待ちのセットでも学習は止まらない。
